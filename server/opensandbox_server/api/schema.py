...
class PoolMemberContainerStatus(BaseModel):
    """Safe diagnostic state for one regular or init container."""

    name: str = Field(..., description="Container name.")
    ready: bool = Field(..., description="Whether Kubernetes reports this container ready.")
    restart_count: int = Field(..., alias="restartCount", description="Container restart count.")
    state: str = Field(..., description="Current container state.")
    reason: Optional[str] = Field(None, description="Kubernetes state reason, when present.")

    class Config:
        populate_by_name = True


class PoolMemberEvent(BaseModel):
    """Safe latest Kubernetes event summary for a non-ready Pool member."""

    type: Optional[str] = Field(None, description="Kubernetes event type.")
    reason: Optional[str] = Field(None, description="Kubernetes event reason.")
    observed_at: Optional[str] = Field(
        None, alias="observedAt", description="Latest event timestamp."
    )
...
class PoolMemberResponse(BaseModel):
    """Owner-verified diagnostic projection of one Pool member Pod."""

    name: str = Field(..., description="Pool member Pod name.")
    phase: str = Field(..., description="Kubernetes Pod phase.")
    ready: bool = Field(..., description="Whether the Pod Ready condition is true.")
    containers: List[PoolMemberContainerStatus] = Field(
        default_factory=list, description="Regular container diagnostics."
    )
    init_containers: List[PoolMemberContainerStatus] = Field(
        default_factory=list, alias="initContainers", description="Init container diagnostics."
    )
    latest_event: Optional[PoolMemberEvent] = Field(
        None, alias="latestEvent", description="Latest event for a non-ready member, when readable."
    )
...
class PoolMembersResponse(BaseModel):
    """Bounded owner-verified Pool member diagnostics."""

    pool_name: str = Field(..., alias="poolName", description="Pool name.")
    member_count: int = Field(
        ..., alias="memberCount", description="Total owner-verified member Pod count."
    )
    truncated: bool = Field(..., description="Whether the requested member limit excluded members.")
    members: List[PoolMemberResponse] = Field(..., description="Owner-verified member diagnostics.")
